...
// Pastikan path action ini sesuai dengan di proyek Anda
...
export default function DrillCore({ unitId, questions }: { unitId: string, questions: Question[] }) {
  ...
  const playSound = (effect: 'correct' | 'wrong' | 'streak' | 'win' | 'lose') => {
    ...
    try {
      // Menggunakan objek Audio baru yang dipaksa muat ulang agar tidak di-block browser
      ...
      if (playPromise !== undefined) {
          playPromise.catch(error => {
              console.warn(`Gagal memutar ${effect}: Mungkin file belum dimuat atau diblokir browser.`);
          });
      }
    } catch (e) {
  ...
  // 🔥 ADAPTOR ANTI-KOSONG (TIDAK MUNGKIN GAGAL) 🔥
      ...
      if (currentQ && currentQ.options) {
         ...
         if (Array.isArray(rawData)) {
             parsedOptions = rawData.map((opt: any, i: number) => {
                 ...
                 return {
                     key: opt.key || opt.id || defaultKey,
                     // DETEKSI SEMUA NAMA FIELD YANG MUNGKIN ADA DI DATABASE ANDA
                     text: opt.text || opt.content || opt.label || opt.title || opt.name || JSON.stringify(opt),
                     score: Number(opt.score || opt.value) || 0
                 };
             });
         }
      }
  ...
  const finishGame = async () => {
    ...
    // Blokir sementara jika file action belum ada agar tidak crash
    try { await submitDrillResult(unitId, score, stars); } catch(e) { console.log(e) }
  };
  ...
  return (
    <div className="max-w-xl mx-auto min-h-[85vh] flex flex-col p-4 pb-48 relative overflow-hidden">
       ...
       <div className="flex-1 relative z-10">
           ...
           {/* 🔥 RENDER OPSI JAWABAN (ANTI-KOSONG) 🔥 */}
               ...
               {parsedOptions.map((opt, i) => {
                   ...
                   return (
                       <button key={i} onClick={() => handleAnswer(opt.key)} disabled={phase === 'FEEDBACK'} className={`w-full p-4 rounded-xl border text-left flex items-center gap-4 transition-all active:scale-[0.98] backdrop-blur-sm ${containerClass}`}>
                           <div className="flex-shrink-0 w-7 h-7 rounded flex items-center justify-center border border-white/10 text-sm font-mono font-bold bg-black/20">
                              {opt.key}
                           </div>
